...
def main_dashboard():
    ...
    tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs([
        "🌐 Standing", "📊 Analytics", "🏥 Experience", "💉 Procedures", "🔬 Academic", "🛡️ Vault"
    ])
    ...
    with tab1:
        ...
        with col_p2:
            summary_text = st.text_area("Professional Summary / Pitch", value=profile[0].get('summary', '') if profile else "", height=125, placeholder="E.g. Internal Medicine Resident with a focus on Interventional Cardiology and QIP...")
    ...
    with tab2:
        st.subheader("Clinical Competency Analytics")
        if procedures:
            df_proc = pd.DataFrame(procedures)
            # Create a pivot for better charting
            chart_data = df_proc.groupby(['procedure', 'level'])['count'].sum().unstack().fillna(0)
            st.bar_chart(chart_data)
            
        else:
            st.info("Log your first procedure in the 'Procedures' tab to generate your analytics dashboard.")
    ...
    with tab4:
        ...
        with st.form("add_proc", clear_on_submit=True):
            ...
            n = col_a.text_input("Procedure Name")
            l = col_b.selectbox("Competency Level", ["Observed", "Supervised", "Independent", "Assessor/Trainer"])
            c = col_c.number_input("Total Count", 1)
    ...
    with tab5:
        ...
        with st.form("add_proj", clear_on_submit=True):
            t = st.selectbox("Type", ["Audit", "Research", "QIP", "Teaching", "Publication"])
    ...
    with tab6:
        st.subheader("🛡️ Secure Vault & Export")
        up_file = st.file_uploader("Upload Evidence (PDF/JPG)", type=['pdf', 'jpg'])
        if up_file and st.button("📤 Sync to Vault"):
            path = f"{st.session_state.user_email}/{up_file.name}"
            client.storage.from_('medical-vault').upload(path, up_file.getvalue())
            st.success("Document Secured.")
        
        st.divider()
        if st.button("🏗️ Compile & Download Full CV"):
            pdf_bytes = generate_pdf(st.session_state.user_email, profile, rotations, procedures, projects, active_countries)
            st.download_button(label="⬇️ Download Professional PDF", data=pdf_bytes, file_name="Medical_Passport_Full.pdf", mime="application/pdf")
