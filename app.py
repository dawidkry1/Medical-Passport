...
# --- GLOBAL MAPPING DATA (UK, Poland, etc.) ---
EQUIVALENCY_MAP = {
...
# --- 2. AUTHENTICATION (FIXED: ENTER TO LOGIN) ---
if 'authenticated' not in st.session_state:
    st.session_state.authenticated = False

def login_screen():
    st.title("🏥 Medical Passport Gateway")
    st.markdown("Please sign in to access your clinical portfolio.")
    
    # Wrap inputs in a form to enable "Enter" key submission
    with st.form("login_form", clear_on_submit=False):
        ...
        if submit_button:
            try:
                ...
                st.error("Credential verification failed. Please check your email and password.")
...
def main_dashboard():
    ...
    with tab1:
        ...
        if st.button("💾 Save Preferences"):
            client.table("profiles").upsert({"user_email": st.session_state.user_email, "global_tier": selected_tier, "selected_countries": active_countries}, on_conflict="user_email").execute()
    ...
    with tab5:
        ...
        if uploaded_file and st.button("📤 Vault File"):
            with st.spinner("Uploading..."):
                path = f"{st.session_state.user_email}/{uploaded_file.name}"
                client.storage.from_('medical-vault').upload(path, uploaded_file.getvalue())
    ...
    with tab6:
        ...
        # RESTORED: Manual choice for export countries
        export_countries = st.multiselect("Include in Header (Choose countries to compare):", 
